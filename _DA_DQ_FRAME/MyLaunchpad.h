...
typedef volatile struct{
    bool    CONTROL;           //! ENABLE/DISABLE SYSTEM
    float   Pref;              //! Reference Active Power
    float   Qref;              //! Reference Reactive Power
    float   VDCI;              //! Input VDC
    bool    CANRUN;            //! RUN status
    bool    OVERCURRENT;       //! Over current status
}ST_STATUS;

#define ST_STATUS_DEFAULT   {0,0,0,0,0,0};
...
inline void Setup_CC_Block(uint8_t chan,uint16_t CompareA,uint16_t CompareB){
    ...
    EPWM_PTR[chan]->CMPCTL.bit.LOADAMODE    = CC_CTR_ZERO;  // Load on zero
    EPWM_PTR[chan]->CMPCTL.bit.LOADBMODE    = CC_CTR_ZERO;
}
